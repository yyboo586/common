...
func (m *Token) GetRequestToken(r *ghttp.Request) (token string) {
	// 请求头获取
	n := len(BearerPrefix)
...
func (m *Token) GetToken(r *ghttp.Request) (tData *TokenData, err error) {
	token := m.GetRequestToken(r)
	tData, _, err = m.GetTokenData(r.GetCtx(), token)
	return
}

func (m *Token) IsLogin(r *ghttp.Request) (b bool, failed *AuthFailed) {
	b = true
	urlPath := r.URL.Path
